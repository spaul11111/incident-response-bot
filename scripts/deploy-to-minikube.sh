...
OVERRIDE_FILE="./helm/values-local-override.yaml"
...
if [ -f "${OVERRIDE_FILE}" ]; then
    echo -e "${GREEN}✅ Using override file with actual tokens${NC}"
    ...
    echo -e "${YELLOW}⚠️  Override file not found. Using template values (tokens may not work)${NC}"
    echo -e "${YELLOW}   Create ${OVERRIDE_FILE} with your actual Slack tokens${NC}"
    helm upgrade --install ${RELEASE_NAME} ${CHART_PATH} \
        --namespace ${NAMESPACE} \
        --values ${VALUES_FILE} \
        --wait \
